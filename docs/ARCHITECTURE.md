...
- `simforge_new/interfaces/cli.py`
  - CLI entry points (e.g. `proto_sim`) that spin up the session, GUI, and background tasks.
  - Provides a headless path for tests by reusing the `SimulationSession` orchestration layer.

- `simforge_new/core/transformations.py`
