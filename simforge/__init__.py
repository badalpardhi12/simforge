...
def run_gui(config: SimforgeConfig, debug: bool = False):
    """Lazy import of wxPython GUI to avoid heavy dependencies at import time."""
    try:
        from .control_gui import run_gui as _run_gui
    except ImportError as exc:
        raise ImportError(
            "wxPython is required for GUI functionality. Install with: pip install wxpython"
        ) from exc
    return _run_gui(config, debug=debug)
