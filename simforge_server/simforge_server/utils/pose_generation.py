...
def transform_pose_to_world(
    ...
    """
    Transform a pose from target object frame to world frame.
    
    Args:
        pose: Pose in target object's local frame
        target_position: Target object position in world frame (x, y, z) meters
        target_orientation_quat_xyzw: Target object orientation in world frame
        
    Returns:
        (position_world, orientation_world_quat_xyzw)
    """
    # Build rotation matrix from target orientation
    ...
    # Transform position: p_world = R_target * p_local + t_target
    p_local = np.array(pose.position_m)
    t_target = np.array(target_position)
    p_world = rot @ p_local + t_target
    
    # Transform orientation: q_world = q_target * q_local
    q_world = _quat_multiply(target_orientation_quat_xyzw, pose.orientation_quat_xyzw)
    q_world = _normalize_quaternion(q_world)
    
    return (tuple(p_world), q_world)
...
def rpy_deg_to_quat_xyzw(roll_deg: float, pitch_deg: float, yaw_deg: float) -> Tuple[float, float, float, float]:
    """Convert RPY angles in degrees to quaternion (x,y,z,w)."""
    return _rpy_to_quat_xyzw(
