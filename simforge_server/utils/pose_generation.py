...
def transform_pose_to_world(
    ...
    """
    Transform a pose from target object frame to the reference frame.
    
    The target_position and target_orientation should come from a TF lookup
    of the target frame relative to the desired reference frame (typically base_link).
    
    Args:
        pose: Pose in target object's local frame
        target_position: Target object position relative to reference frame (x, y, z) meters
        target_orientation_quat_xyzw: Target object orientation relative to reference frame
        
    Returns:
        (position_in_ref_frame, orientation_in_ref_frame_quat_xyzw)
    """
