...
from typing import Dict, List, Optional, Tuple, TYPE_CHECKING, Any, Sequence
...
class SessionController:
    ...
    def environment_objects(self) -> List[str]:
        names: List[str] = []
        for obj in self.spec.world.objects:
            if obj.name:
                names.append(obj.name)
        return sorted(names)

    def execute_proto_sim(
        self,
        robot_name: str,
        parameters: ProtoSimParameters,
        object_names: Sequence[str],
        *,
        idle_timeout: float = 15.0,
        progress: Optional[ProgressCallback] = None,
    ) -> "asyncio.Future[ProtoSimRunResult]":
        if self._session is None:
            raise RuntimeError("Simulation session is not initialised")
        if robot_name not in self._profiles:
            raise ValueError(f"Unknown robot '{robot_name}'")
        if not object_names:
            raise ValueError("object_names must not be empty")

        robot_profile = self._profiles[robot_name]
        frames = self._session.reference_frames()
        frame_lookup: Dict[str, str] = {}
        for key in frames.keys():
            if key.startswith("obj:"):
                frame_lookup[key.split(":", 1)[1]] = key

        selected_frames: Dict[str, str] = {}
        for name in object_names:
            lookup_key = name.lower().strip()
            frame_key = frame_lookup.get(lookup_key)
            if not frame_key:
                continue
            selected_frames[name] = frame_key

        if not selected_frames:
            raise ValueError("No valid reference frames found for the selected objects")

        poses = generate_proto_poses(parameters)
        coro = execute_proto_sim(
            self._session,
            robot_profile,
            frames,
            selected_frames,
