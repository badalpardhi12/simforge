...
def _resolve_path(path: Optional[str], base_dir: Path) -> Optional[str]:
    ...
    # Also try from the simforge_genesis package root
    package_root = Path(__file__).resolve().parents[2]  # simforge_genesis → repo root
    simforge_genesis_root = Path(__file__).resolve().parents[1]  # simforge_genesis/
    if simforge_genesis_root not in search_roots:
        search_roots.append(simforge_genesis_root)
